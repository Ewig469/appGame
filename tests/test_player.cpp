...
void test_initialization_contract(const preset::BoardConfig& config) {
    ...
    require_exception([&] { player.request(); },
    ...
    require_exception(
        [&] {
            preset::BoardConfig invalid_config = config;
            invalid_config.width = 4;
            bruecken::RandomAIPlayer invalid_player;
            invalid_player.init(invalid_config, 1);
        },
        "init must reject an invalid board configuration");

    player.init(config, 1);
    require_exception([&] { player.init(config, 1); },
...
void test_random_ai_turn_contract(const preset::BoardConfig& config) {
    ...
    require_exception([&] { player_two.request(); },
    ...
    require(first_move->get_id() == 1,
            "Random AI must return its own player ID");
    require_exception([&] { player_one.request(); },
